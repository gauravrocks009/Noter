...
public partial class MainWindow : Window
{
    ...
    private string GetNotePath(Note note)
    {
        if (string.IsNullOrEmpty(note.FolderName))
            return System.IO.Path.Combine(_appDataPath, $"{note.Id}.md");
        return System.IO.Path.Combine(_appDataPath, note.FolderName, $"{note.Id}.md");
    }

    private void MigrateRtfToMd(string dirPath)
    {
        try
        {
            ...
            foreach (var file in files)
            {
                try
                {
                    ...
                    string markdown = MarkdownHelper.FlowDocumentToMarkdown(doc);
                    string mdFile = System.IO.Path.ChangeExtension(file, ".md");
                    System.IO.File.WriteAllText(mdFile, markdown, System.Text.Encoding.UTF8);
    ...
    private void LoadNotes()
    {
        ...
        // Migrate RTF to Markdown on startup
        MigrateRtfToMd(_appDataPath);
        foreach (var dir in System.IO.Directory.GetDirectories(_appDataPath))
        {
            MigrateRtfToMd(dir);
        }
    ...
    {
        var files = System.IO.Directory.GetFiles(dirPath, "*.md");
        foreach (var file in files)
        {
            var note = new Note
            {
            ...
            {
                var doc = new FlowDocument();
                string markdown = System.IO.File.ReadAllText(file, System.Text.Encoding.UTF8);
                MarkdownHelper.MarkdownToFlowDocument(markdown, doc);
    ...
    private void NotesList_SelectionChanged(object sender, System.Windows.Controls.SelectionChangedEventArgs e)
    {
        if (NotesList.SelectedItem is Note note)
        {
            ...
            if (System.IO.File.Exists(file))
            {
                ...
                {
                    string markdown = System.IO.File.ReadAllText(file, System.Text.Encoding.UTF8);
                    MarkdownHelper.MarkdownToFlowDocument(markdown, EditorBox.Document);
                }
                catch
                {
                    EditorBox.Document.Blocks.Clear();
    ...
    private void EditorBox_TextChanged(object sender, System.Windows.Controls.TextChangedEventArgs? e)
    {
        ...
        string markdown = MarkdownHelper.FlowDocumentToMarkdown(EditorBox.Document);
        System.IO.File.WriteAllText(file, markdown, System.Text.Encoding.UTF8);
